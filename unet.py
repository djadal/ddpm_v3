...
class Unet1D(Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out, seq) in enumerate(in_out):
            ...
            self.embed_layers.append(nn.Embedding(num_embeddings=dim_in, embedding_dim=64))
        ...
        self.fusion_layer = nn.AdaptiveAvgPool1d(1)
        ...
        for ind, (dim_in, dim_out, seq) in enumerate(reversed(in_out)):
            ...
            self.embed_layers.append(nn.Embedding(num_embeddings=dim_out, embedding_dim=64))
    ...
    def time_embedding(self, device=None, shape=None, dim=128):
        B, C, L = shape
        pe = torch.zeros(B, L, dim).to(device) 
        ...
        div_term = 1 / torch.pow(
            10000.0, torch.arange(0, dim, 2).to(device) / dim
        )
    ...
    def get_side_info(self, device, shape, layer_idx):
        B, C, L = shape

        # get time_embed
        time_embed = self.time_embedding(device=device, shape=shape)  # -> (B, L, t_d)
        time_embed = time_embed.unsqueeze(1).expand(-1, C, -1, -1) # -> (B, C, L, t_d)

        # get feature_embed
        feature_embed = self.embed_layers[layer_idx](torch.arange(C).to(device))  # -> (C, f_d)
        feature_embed = feature_embed.unsqueeze(0).expand(B, -1, -1)  # -> (B, C, f_d)
        feature_embed = feature_embed.unsqueeze(2).expand(-1, -1, L, -1) # -> (B, C, L, f_d)
