...
if __name__ == '__main__':
    ...
    # Trainer
    parser.add_argument("--train_steps", type=int, default=40000)
    ...
    parser.add_argument("--resume", type=int, default=4, help='Resume model from checkpoint')
    ...
    if args.status == 'train':
        trainer.load(args.resume, args.sampling_timesteps, args.status)
        trainer.train()

    elif args.status == 'test':
        trainer.load(args.resume, args.sampling_timesteps, args.status)
        val_loader = DataLoader(val_set, batch_size=16, shuffle=False, pin_memory=True, drop_last = True)
        val_loaer = trainer.accelerator.prepare(val_loader)
        trainer.evaluate(val_loader, criterion=trainer.criterion, num_batches=None)
