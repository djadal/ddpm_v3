...
class GaussianDiffusion1D(Module):
    ...
    def p_mean_variance(self, x, t, x_self_cond=None, clip_denoised=True, reference=None):
        preds = self.model_predictions(x, t, x_self_cond, reference=reference)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, x_self_cond=None, clip_denoised=True, reference=None):
    ...
    @torch.no_grad()
    def sample(self, batch_size=16, condition=None, reference=None):
        seq_length, channels = self.seq_length, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn((batch_size, channels, seq_length), condition, reference)
    ...
    def p_losses(self, x_start, t, noise=None, condition=None, reference = None):
        b, c, n = x_start.shape
        noise = default(noise, lambda: torch.randn_like(x_start))  #

        # noise sample
        x = self.q_sample(x_start=x_start, t=t, noise=noise)  # x_0 -> x_t
        ...
        model_out = self.model(x, t, x_self_cond, reference)  # x_t -> noise
    ...
    def forward(self, img, cond, ref, *args, **kwargs):
        ...
        return self.p_losses(img, t, condition=cond, reference=ref, *args, **kwargs)
