...
class ST_MEM(nn.Module):
    def __init__(self,
        ...
        self.to_decoder_embedding = nn.Linear(embed_dim, decoder_embed_dim) # (b, (c, n), decoder_embed_dim)

        self.mask_embedding = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim)) # (1, 1, decoder_embed_dim)
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        x = rearrange(x, 'b (c n) p -> b c n p', c=self.num_leads)# (b, c, n*ratio+2, d)
        b, _, n_masked_with_sep, d = x.shape # n_masked_with_sep = n * masked_ratio
        n = ids_restore.shape[2]
        mask_embeddings = self.mask_embedding.unsqueeze(1) # (1, 1, 1, d)
